...
public enum NodeType
{
    ...
    /// <summary>
    /// <c>CREATE SEQUENCE &lt;name&gt; [options]</c>. <c>leftAst</c> is the name; <c>rightAst</c> is
    /// the flat option word chain, or null when no options were written.
    ///
    /// <para>Appended at the end for the reason given on <see cref="TypeBytesSized"/>: member
    /// ordinals cross the wire, so no member may be inserted mid-enum.</para>
    /// </summary>
    ...
    /// <summary><c>CREATE SEQUENCE IF NOT EXISTS</c>; the same children as <see cref="CreateSequence"/>.</summary>
    CreateSequenceIfNotExists,

    /// <summary><c>DROP SEQUENCE &lt;name&gt;</c>. <c>leftAst</c> is the name.</summary>
    DropSequence,

    /// <summary><c>DROP SEQUENCE IF EXISTS &lt;name&gt;</c>. <c>leftAst</c> is the name.</summary>
    DropSequenceIfExists,

    /// <summary>
    /// <c>ALTER SEQUENCE &lt;name&gt; &lt;options&gt;</c>. <c>leftAst</c> is the name;
    /// <c>rightAst</c> is the option word chain, which this form always has.
    /// </summary>
    AlterSequence,

    /// <summary><c>ALTER SEQUENCE &lt;name&gt; RENAME TO &lt;name&gt;</c>. Metadata-only.</summary>
    AlterSequenceRenameTo,

    /// <summary>
    /// One link of a sequence option chain: <c>leftAst</c> is everything read so far and
    /// <c>rightAst</c> is the word or number just read. Left-deep, like every other list the
    /// grammar builds.
    /// </summary>
    SequenceOptionList,

    /// <summary>
    /// One word of a sequence option list, its text in <c>yytext</c> and no children. A number in
    /// that position builds an <see cref="Integer"/> node instead, so the validator can tell a value
    /// from a keyword without re-lexing.
    /// </summary>
    SequenceOptionWord,

    /// <summary><c>SHOW SEQUENCES [LIKE '&lt;pattern&gt;']</c>. <c>leftAst</c> is the pattern, or null.</summary>
    ShowSequences,

    /// <summary><c>SHOW CREATE SEQUENCE &lt;name&gt;</c>. <c>leftAst</c> is the name.</summary>
    ShowCreateSequence,

    /// <summary><c>COMMENT ON SEQUENCE &lt;name&gt; IS &lt;value&gt;</c>. A null <c>rightAst</c> removes the comment.</summary>
    CommentOnSequence,

    /// <summary>
    /// The column constraint written as <c>GENERATED ALWAYS AS IDENTITY</c>: the column draws from
    /// a sequence created and owned for it, and a user-supplied value is refused.
    /// </summary>
    ConstraintIdentityAlways,

    /// <summary>
    /// The column constraint written as <c>GENERATED BY DEFAULT AS IDENTITY</c>, and what
    /// <c>SERIAL</c> desugars to: the column draws from a sequence created and owned for it, but a
