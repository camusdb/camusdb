...
public sealed class BPlusTreeEntry<TKey, TValue> where TKey : IComparable<TKey> where TValue : IComparable<TValue>
{
    ...
    public AsyncLazy<BPlusTreeNode<TKey, TValue>?> Next { get; set; } // helper field to iterate over array entries    

    public BPlusTreeEntry(TKey key, IBPlusTreeNodeReader<TKey, TValue>? reader, BPlusTreeNode<TKey, TValue>? next)
    {
        Key = key;
        Reader = reader;

        if (next is not null)
            Next = new AsyncLazy<BPlusTreeNode<TKey, TValue>?>(() => Task.FromResult<BPlusTreeNode<TKey, TValue>?>(next));
        else
            Next = new AsyncLazy<BPlusTreeNode<TKey, TValue>?>(LoadNode);
    }
    ...
    private async Task<BPlusTreeNode<TKey, TValue>?> LoadNode()
    {
    ...
    {
        Console.WriteLine("CanBeSeenBy={0} {1}", timestamp, mvccValues.Count);
        ...
        foreach (KeyValuePair<HLCTimestamp, BTreeMvccEntry<TValue>> keyValue in mvccValues)
        {
            Console.WriteLine("CanBeSeenBy {0} {1} {2}", keyValue.Key, keyValue.Value.CommitState, keyValue.Value.Value);
